...
import { ArrowLeft, Upload, Check, Loader2, MapPin, X, Move, Image, Trash2, AlertCircle } from 'lucide-react'
import { useApp } from '../../context/AppContext'
import { updateLocationInApi } from '../../utils/apiClient'

export default function AdminLocationMap() {
  const { locationId } = useParams()
  const { spaces, locations, setLocations } = useApp()
  const mapRef = useRef(null)

  const location = locations.find(l => l.id === locationId)
  const locationSpaces = spaces.filter(s => s.active && s.locationId === locationId)

  const [backgroundImage, setBackgroundImage] = useState(location?.floorMap?.backgroundImage || null)
  const [pins, setPins] = useState(location?.floorMap?.pins || [])
  ...
  if (!location) {
    return (
      <div className="max-w-7xl mx-auto px-4 py-16 text-center">
        ...
        <Link to="/admin/locations" className="btn-secondary text-sm">Back to locations</Link>
      </div>
    )
  }
  ...
  const handleSave = async () => {
    ...
    setSaved(false)
    setSaveError('')
    const floorMap = { backgroundImage, pins }
    const { error } = await updateLocationInApi(locationId, { floor_map: floorMap })
    if (error) {
      setSaveError(error)
    } else {
      setLocations(prev => prev.map(l => l.id === locationId ? { ...l, floorMap } : l))
      setSaved(true)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-8">
      <div className="flex items-center gap-4 mb-6">
        <Link to="/admin/locations" className="btn-secondary text-sm">
          <ArrowLeft className="w-4 h-4" /> Back
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
            <MapPin className="w-6 h-6 text-blue-600" /> Floor Map — {location.name}
          </h1>
          <p className="text-sm text-gray-500 mt-0.5">
            Upload a floor plan image, then place pins for each space at this location.
          </p>
        </div>
        <div className="flex items-center gap-3">
          ...
          <button onClick={handleSave} disabled={saving} className="btn-primary">
            {saving
              ...
              : <><Check className="w-4 h-4" />Save Map</>
            }
          </button>
        </div>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="card p-4">
            ...
            <p className="text-xs text-gray-400 mb-3">
              {activePlaceId ? 'Click on the map to place this pin' : 'Select a space to place its pin'}
            </p>
            {locationSpaces.length === 0 ? (
              <p className="text-xs text-gray-400">No active spaces assigned to this location.</p>
            ) : (
        ...
        <div className="card overflow-hidden">
          {!backgroundImage ? (
            ...
            <div
              ...
              style={{ minHeight: 300 }}
            >
              <img src={backgroundImage} alt="Floor plan" className="w-full h-auto block" draggable={false} />
              {pins.map(pin => {
